...
promise_test(async testCase => {
  ...
  // called.
  const txn2 = db.transaction(['books'], 'readwrite');
  const objectStore1 = txn2.objectStore('books');
  objectStore1.put({isbn:'two', title:'title2'});
  const addRequest = objectStore1.add({isbn:'one', title:'title2'});
  txn2.commit();
  ...
  await txnWatcher.wait_for('error');
  ...
  const objectStore2 = txn3.objectStore('books');
  const getRequest1 = objectStore2.get('one');
  const getRequest2 = objectStore2.count('two');
...
promise_test(async testCase => {
  ...
  const objectStore1 = txn2.objectStore('books');
  objectStore1.put({isbn: 'two', title:'title2'});
  const addRequest = objectStore1.add({isbn: 'one', title:'unreached_title'});
  ...
  // Wait for the transaction to complete. We have to explicitly wait for the
  // error signal on the transaction because of the nature of the test tooling.
  const txnWatcher = transactionWatcher(testCase, txn2);
  await txnWatcher.wait_for('error');
  await txnWatcher.wait_for('complete');
